...
class TabnakSpider(scrapy.Spider):
    ...
    custom_settings = {
        ...
        'FEEDS': {
            'Tabnak_Scrapy_Dataset.csv': { # نام و مسیر فایل خروجی (در پوشه اصلی پروژه ایجاد می‌شود)
                ...
                'fields': ['title', 'abstract', 'body', 'date_georgian_iso', 'link', 'category']
            }
        },
        ...
        'DOWNLOAD_DELAY': 0.1,                   
        'USER_AGENT': "TabnakCrawler/1.0 (+https://github.com/your-project)",
        'ROBOTSTXT_OBEY': True,                  
    }
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            ...
            self.logger.info(f"Crawl range set from command line: {self.start_id} to {self.end_id}")
        except ValueError:
    ...
    def extract_and_convert_date(self, response):
        ...
        if date_en_tag:
            ...
            try:
                ...
                self.logger.warning(f"Date format error for URL {response.url}: '{raw_date}'")
            finally:
    ...
    def parse(self, response):
        ...
        item = NewsItem()
        allowed_categories = ["ورزشی", "اقتصادی", "بین‌الملل"]
        ...
        if category_tag:
            ...
            if category_name not in allowed_categories:
                self.logger.debug(f"Skipping {response.url}: Category '{category_name}' is not in allowed list.")
                return # نادیده گرفتن خبر و توقف پردازش
        else:
            self.logger.debug(f"Skipping {response.url}: Could not find news category tag.")
        ...
        if id_match:
            ...
            self.logger.warning(f"Could not reliably extract ID from URL: {response.url}. Using raw URL for link field.")
